...
Replace #189 before use. Do not paste the issue body into this
...
Audit the implementation for GitHub Issue #189 from the current
...
1. Fetch GitHub Issue #189 before planning.
...
- Fix only confirmed gaps against Issue #189 or repository
  invariants.
...
1. Make only issue-scoped audit fixes.
2. Commit with a Conventional Commit message that references
   #189.
3. Land audit fixes directly on `dev`; do not require a human reviewer.
4. Diagnose and repair CI failures with bounded attempts. Stop after three
   failed CI repair attempts and report the blocker.
5. Required `ci` must be green after the audit landing.
