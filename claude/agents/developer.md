...
hooks:
  PreToolUse:
    - matcher: "Bash"
      hooks:
        - type: command
          command: "jq -r '.tool_input.command // empty' | grep -E '^(rm -rf|git push --force|git reset --hard|DROP TABLE)' && echo '[developer] Destructive command detected - ensure this is intentional' >&2 || exit 0"
  Stop:
    - hooks:
        - type: command
