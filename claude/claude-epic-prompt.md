# Project-neutral epic-driven autonomous workflow prompt

> **Usage**: replace the placeholder in the run-card below before pasting. The GitHub epic and its child issues remain
> the source of truth for scope, order, acceptance criteria, and closure evidence.
...
You are the **coordinator** for the project-scoped Claude agent cluster in the current repository.
...
Drive **Epic #532** autonomously from its current state to a state where the epic is closure-ready and handed over
...
Claude's epic handoff is complete only when all of these are true:

1. Epic #532 has been read fully, including body, comments, labels, child issues, linked PRs, and delivery-board
   state.
...
8. Epic #532 remains open and is marked `Ready for Human Review` with a clear closure request comment linking the
   final epic PR.
...
1. Fetch Epic #532 with the GitHub plugin or `gh issue view`.
...
4. Create every child issue branch from the current epic branch, not directly from the integration branch:
  - `claude/issue-{ISSUE_ID}-<kebab-summary>`
...
Use issue-closing keywords carefully. Prefer `Refs #{ISSUE_ID}` or `Part of #532` on child PRs into the epic
...
4. Detect safe parallelism only when issues have disjoint file ownership, independent acceptance criteria, and no
   ordering dependency.
5. For parallel work, assign one Claude team per issue and explicitly define write ownership before any write work
   starts.
...
10. Prefer `Refs #{ISSUE_ID}` or `Part of #532` for child PRs into the epic branch. Use `Closes #{ISSUE_ID}` only
