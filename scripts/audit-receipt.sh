...
#
# --findings / --user-facing are optional and default to "unknown". They are the
# inputs to the epic-merge auto-merge decision, so omitting them is fail-closed:
# the merge gate will NOT auto-merge into an epic branch unless findings=0 and
# user_facing=false. A standalone audit can omit them safely.
